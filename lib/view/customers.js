    ...
    "customers": (function () {
        ...
        var route = 'customers',
            ...
            view = new child_view({
                        ...
                        items: [ // accordion cells section
                            {
                                ...
                                text: "Assigned Servers", // string, header's text (html allowed)
                                //icon:   "bid.png", // string, header's icon, optional
                                open: false, // boolean, true to open/false to close item on init
                                //height: 200         // number, cell's height (multimode only)
                            }, {
                                id: "c", // item id, required
                                text: "Assigned IPs", // string, header's text (html allowed)
                                //icon:   "bid.png", // string, header's icon, optional
                                open: false, // boolean, true to open/false to close item on init
                                //height: 200         // number, cell's height (multimode only)
                            }, {
                                id: "d", // item id, required
                                text: "Notes", // string, header's text (html allowed)
                                //icon:   "bid.png", // string, header's icon, optional
                                open: false, // boolean, true to open/false to close item on init
                                //height: 200         // number, cell's height (multimode only)
                            }
                ...
                _grid_notes: function() {
                    var self = this,
                        wrapper;
                    if (self.grid_notes === null) {

                        wrapper = self.form.getContainer("notes");

                        wrapper.style.height = "150px";
                        
                        self.grid_notes = new dhtmlXGridObject( wrapper.id );

                        self.grid_notes.setHeader(self._settings.grid_notes.header.join(','));
                        self.grid_notes.setColumnIds(self._settings.grid_notes.id.join(','));
                        self.grid_notes.enableAutoWidth(true);
                        self.grid_notes.setInitWidths(self._settings.grid_notes.width.join(','));
                        //console.log( self._settings.grid.id.join(',') );
                        //console.log( self._settings.grid.width.join(',') );
                        self.grid_notes.setColAlign(self._settings.grid_notes.colAligns.join(','));
                        self.grid_notes.setColTypes(self._settings.grid_notes.colTypes.join(','));
                        self.grid_notes.enableMultiline(false);
                        
                        self.grid_notes.init();
                        self.grid_notes.enableEditEvents(false, false, false);


                        self.grid_notes.attachEvent("onRowSelect", self.presenter.gridOnRowSelectHandler);
                    }
                ...
                _form_item: function( model ) {
                    ...
                    self._settings.grid.id.forEach(function(id, index) {
                        ...
                        if( id == '__v' || id == '_id'  || id == 'assigned_servers'   || id == 'archive'   || id == 'notes' )
                            return;
                ...
                _form: function( container, action ) {
                    ...
                    for (var prop in defaults) {
                        ...
                        if (prop != '_id' && prop != '__v' && prop != 'assigned_servers' && prop != 'notes' ) {
                            input_settings = {
